# 数据契约 v2.2（docs/specs/data-contract.md）
...
```json
{
  ...
  "briefs": [{ "name": "简讯条目", "one_liner": "一句话（描述先行、自洽；不以\"采集时间…\"开头，star 等证据置句尾，见 §2.2）", "link": "URL" }],
  "source_gaps": ["本期缺失的来源及原因（无则空数组）"]
}
```

## 2.2 overview 概览字段规范（v2.2）
...
**briefs[].one_liner（内容规范，schema 不变）**
...
- overview 的 brief 概览文案：使用 `one_liner`（按上文已是干净自洽句）。
- 站点必须容忍 `gist` 缺失（向前/向后兼容）；`gist` 存在时不得再对其做截断。

**存量回填**
- `2026-07-02`、`2026-07-03` 两期按本节为每个 highlight 补 `gist`、并把 briefs 的 `one_liner` 清洗为描述先行；均只依据各条目既有 `summary`/`evidence_notes`/`one_liner` 改写，不新增事实、不改动其它字段。
