...
| Phase 8 | 看板简讯留白根治：泛化审查看板算法 → 决策文档定验收契约 → 单窗口实施 | 完成：实施 `022ca1c` + 决策文档 `3f2d9a0`；单连续列表 + 逐条 `hidden` + 按钮入网格 + 奇偶整行（`is-wide`/`is-complete-odd`）消灭内部空洞，拆分类目/简讯折叠基准、动态「回到前 N 条」文案；仅 `site/app.js`+`styles.css`；已推送 `f2a6ee4..3f2d9a0`，Pages 部署标记已线上验证生效 |
| Phase 9 / Round 2 | 看板二轮线上反馈：Q1 类目卡收起按钮置底（统一 disclosure）、Q2 简讯看板消费 `gist`（契约 v2.3）、类目卡等高拉伸缓解、跨文件耦合注释；两窗口并行（B=简讯 gist SOP+回填；C=站点） | 完成：契约 v2.3 `c419bd0`；数据/SOP `6f45bed`（brief gist 07-02 7/7、07-03 5/5）；站点 `4a519f9`（统一 `overviewDisclosureList`、brief gist 消费 + 启发式降级、`align-items:start`、常量注释）；已推送 `c419bd0..4a519f9`，Pages 部署标记已线上验证生效 |
| 审查 + Round 1 · 信任地基 | 4 维并行只读审查发现 P0「事实可信度破防」→ 确定性校验器 + CI 关口 + 契约 v2.4 锚定星数 + 回港校正两期 + CI 真实 GitHub API 星数复查（收口为 `latest`）；首次生产运行 07-06 在最新严规则下产出并经离线+在线双校验属实 | 完成：栈 `df2da2e`(审查)→`8ad37d4`→`70b0b8a`→`29c8424`→`807a9f7`→`8218769`→`bf552c9`，全推送并 CI 全绿 |
...
- 情报站看板 Round 2（Phase 9）：Phase 8 上线后线上四项反馈。**Q1**（类目卡展开后收起按钮卡中间=割裂感）根因：Phase 8 只改了简讯卡，类目卡仍走旧「button + panel 双结构」。**Q2**（简讯看板文案被截断）根因：简讯看板用 `one_liner` 套 2 行 `line-clamp`，而 `one_liner` 含证据尾——与 highlights 同源，简讯此前无 `gist`。契约升级 **v2.3**（架构师）：briefs 新增可选 `gist`（看板自洽短句、无证据、不截断），`one_liner` 保留为详情简讯区正文；决策落盘 `docs/specs/overview-board-round2-decisions.md`（Q1/Q2 消费/常量注释/等高拉伸验收契约）。两窗口并行（master、文件不交叉）：B（`6f45bed`）SOP 补 brief gist 生成/校验/Markdown 禁入规范 + 回填两期（07-02 7/7、07-03 5/5，均据 `one_liner` 提炼、无新事实、`one_liner` 不动）；C（`4a519f9`）统一 `overviewDisclosureList`（类目/简讯共用「单列表 + 按钮末项」，收起按钮恒置底）、overview 消费 brief `gist`（`is-gist` 不截断，缺失走 `briefSemanticExcerpt` 剥离证据尾降级、详情区仍用完整 `one_liner`）、`.overview-grid` 加 `align-items:start` 缓解等高拉伸、常量旁加 640px 耦合注释。架构师双 diff 自查通过：无 dangling 引用（旧 `overviewDisclosure`/`overview-extra` 已清）、Phase 8 奇偶占位（`is-wide`/`is-complete-odd`）保留、brief gist 事实纪律通过。遗留非阻断：无 `gist` 时的 `one_liner` 启发式剥离对异常句式不完美（有 `gist` 时不受影响）。
- 情报站看板 Phase 8：用户实测简讯卡右下角恒留白。根因=「简讯是唯一两列网格 × 默认 3 条（奇数）不整除 → 右下角恒空一格」+「双网格结构（基准列表 + 独立 extra 网格）补一处漏一处」。泛化审查另发现：魔法数字 `OVERVIEW_BULLET_LIMIT` 单/多列共用耦合、收起文案写死「前 3 条」、`.overview-grid` 等高拉伸留白（本轮不做仅记录）。诊断+验收契约落盘 `docs/specs/overview-board-layout-fix.md`（架构师定「达到什么效果」，实现路径由子窗口自选）。单窗口实施（`022ca1c`）：拆 `OVERVIEW_CATEGORY_BULLET_LIMIT=3`/`OVERVIEW_BRIEF_BULLET_LIMIT=4`；简讯改**单连续 `<ul>` + 逐条 `hidden` 切换**、展开/收起按钮作为网格 `<li>`；按奇偶令落单元素（`is-wide` 按钮 / `is-complete-odd` 末条）`grid-column:1/-1` 占满整行，折叠/展开 × 桌面(≥640px 2 列)/移动(1 列) × 3/4/5/7 条均无内部空洞；动态「回到前 `baseCount` 条」。架构师自查通过（推演全部奇偶情形；确认整行修正与 2 列规则同处 `@media(min-width:640px)`，无断点错配）。遗留非阻断：`OVERVIEW_BRIEF_DESKTOP_COLUMNS=2` 与 CSS `repeat(2)` 跨文件奇偶耦合，建议常量旁加注释指向该断点。
...
- 2026-07-04（凌晨）：看板 Round 2（Phase 9）。契约 v2.3（`c419bd0`）+ 双线并行（B=`6f45bed` 简讯 gist SOP/回填；C=`4a519f9` 站点统一 disclosure + gist 消费 + `align-items:start` + 常量注释）已推送 `c419bd0..4a519f9`。架构师双 diff 自查通过、brief gist 事实纪律通过。线上验证：Pages 部署后复核标记生效。
- 2026-07-04（凌晨）：Phase 8 看板留白根治。诊断+泛化审查落盘 `docs/specs/overview-board-layout-fix.md`；单窗口实施（`022ca1c`：单连续列表 + 逐条 hidden + 按钮入网格 + 奇偶整行修正）+ 决策文档（`3f2d9a0`）已推送 `f2a6ee4..3f2d9a0`。架构师 diff 自查通过。线上验证：Pages 部署约 100s 后 `is-wide`/`is-complete-odd`/`OVERVIEW_BRIEF_BULLET_LIMIT`/`overview-more-item` 标记均已在线上 app.js/styles.css 生效。
...
- 2026-07-04：自动化流水线 4 维并行只读审查 → 汇总决策简报（`df2da2e`）。用户拍板仅开 Round 1（信任地基），存量 R1 后回港校正。Round 1 落库：确定性校验器 + CI 关口（`8ad37d4`）→ code-review 后硬化（`70b0b8a`）→ 契约 v2.4 锚定星数（`29c8424`）→ 回港校正 07-02/07-03（`807a9f7`，改锚定文案、降级超阈社区项）→ CI 接入真实 GitHub API 星数复查（`8218769`）。一次性推送整栈，CI `validate` + `--github-recheck all` 全绿——回港星数经真实 API 机器核实属实，信任地基首次闭环。
- 2026-07-06：CI 复查收口 `--github-recheck all`→`latest`（`bf552c9`）：只核验当期新日报，不再拿历史冻结快照比实时涨星（消除约 1–2 周后老快照越过 5% 漂移阈值的误红），并与 SOP §10 对齐。同日用户换有额度账户重绑 Automation 并 run now：自动化产出 07-06 日报（`f86673c`，Cursor Agent）。架构师核验：离线校验 3 期 0 error（仅 claude-code >12 万人工复核 WARN）、在线复查 07-06 星数属实（codex 95,766 / open-design 75,346 等均与实时吻合）、CI `notify-wechat` 全绿并推送微信——**首次生产运行在最新更严规则下机器核验通过**。07-04 未产出的原因是旧绑定账户额度不足（非管道缺陷）。收口 push 的 notify 因无报告变更正确 skip，无误报。
